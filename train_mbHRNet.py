...
if __name__=="__main__":
    ...
    #net= Mul(1, 1)
    net=torch.nn.DataParallel( get_pose_net(config.cfg, 1), device_ids=[0, 1, 2, 3])
    ...
    batch_size = 48  # e.g. 16
    lr = 0.001  # e.g. 0.01
    N_train = len(train_img_masks)
    model_save_path = './results_mbHRNet/'  # directory to same the model after each epoch.
    if not os.path.isdir(model_save_path):
        os.mkdir('results_mbHRNet')
    ################################################ [TODO] ###################################################
